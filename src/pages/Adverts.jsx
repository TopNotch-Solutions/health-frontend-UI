...
export default function Adverts() {
  ...
  const fetchAdverts = async () => {
    ...
    try {
      const response = await fetchJSON(
        "http://13.61.152.64:4000/api/portal/adverts/retrieve-all-adverts",
        "GET"
      );
      if (response.adverts) {
  ...
  const handleCreateAdvert = async () => {
    ...
    try {
      const response = await fetchFormData(
        "http://13.61.152.64:4000/api/portal/adverts/create-adverts",
        "POST",
        formData
      );
  ...
  const handleUpdateDescription = async () => {
    ...
    try {
      const response = await fetchJSON(
        `http://13.61.152.64:4000/api/portal/adverts/update-description/${currentAdvertId}`,
        "PATCH",
  ...
  const handleUpdateImage = async () => {
    ...
    try {
      const response = await fetchFormData(
        `http://13.61.152.64:4000/api/portal/adverts/update-image/${currentAdvertId}`,
        "PATCH",
        formData
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetchJSON(
        `http://13.61.152.64:4000/api/portal/adverts/remove-advert/${id}`,
        "DELETE"
      );
                    ...
                    <Card>
                      {advert.image && (
                        <CardMedia
                          component="img"
                          height="160"
                          image={`http://13.61.152.64:4000/adverts/${advert.image}`}
