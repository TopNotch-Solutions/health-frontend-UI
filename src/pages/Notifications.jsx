...
function Notifications() {
    ...
    const fetchNotifications = async () => {
      ...
      try {
        const response = await fetchJSON(
          `https://apihealthconnect.kopanovertex.com/api/portal/notification/all-notifications/${currentUser._id}`,
          'GET'
        );
  ...
  useEffect(() => {
    const fetchAppUsers = async () => {
      if (sendDialogOpen && notificationForm.recipientType === 'single') {
        try {
          const response = await fetchJSON(
            'https://apihealthconnect.kopanovertex.com/api/app/auth/all-users',
            'GET'
          );
  ...
  const handleMarkAsRead = async (id) => {
    try {
      const response = await fetchJSON(
        `https://apihealthconnect.kopanovertex.com/api/portal/notification/mark-read/${id}`,
        'PUT'
      );
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetchJSON(
        `https://apihealthconnect.kopanovertex.com/api/portal/notification/delete/${id}`,
        'DELETE'
      );
      if (response.status === true) {
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      const updatePromises = unreadNotifications.map(notification =>
        fetchJSON(
          `https://apihealthconnect.kopanovertex.com/api/portal/notification/mark-read/${notification.id || notification._id}`,
          'PUT'
        )
      );
    ...
    if (result.isConfirmed) {
      try {
        const deletePromises = notifications.map(notification =>
          fetchJSON(
            `https://apihealthconnect.kopanovertex.com/api/portal/notification/delete/${notification.id || notification._id}`,
            'DELETE'
          )
        );
    ...
    try {
      ...
      if (notificationForm.recipientType === 'all') {
        response = await fetchJSON(
          'https://apihealthconnect.kopanovertex.com/api/portal/notification/send-to-all-users',
          'POST',
          {
        ...
        response = await fetchJSON(
          'https://apihealthconnect.kopanovertex.com/api/portal/notification/send-to-user',
          'POST',
          {
